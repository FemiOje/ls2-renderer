...
fn test_page_0_battle_interface() {
    ...
    assert!(contains_pattern(@svg, @"rect width=\"71\" height=\"71\""), "Should contain equipment slots");
...
fn test_page_1_empty_background() {
    ...
    assert!(contains_pattern(@svg, @"<rect width=\"567\" height=\"862\""), "Should contain main container");
...
fn test_render_page_json() {
    ...
    assert!(contains_pattern(@json_result, @"data:application/json;base64,"), "Should be JSON data URI");
...
fn test_page_image_generation() {
    ...
    assert!(contains_pattern(@page_0_image, @"data:image/svg+xml;base64,"), "Page 0 should be SVG data URI");
    assert!(contains_pattern(@page_1_image, @"data:image/svg+xml;base64,"), "Page 1 should be SVG data URI");
    
    // Page 0 should be longer (more content) than page 1
    assert!(page_0_image.len() > page_1_image.len(), "Battle page should have more content than empty page");
}