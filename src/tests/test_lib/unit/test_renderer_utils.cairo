...
use death_mountain_renderer::utils::renderer::core::text_utils::generate_adventurer_name_text;
...
fn test_animation_scalability_with_different_page_counts() {
    ...
    // (We can't easily test 4+ pages without extending the PageType enum, but we can verify the
    // math)
...
/// @dev Even though we're limited to 3 pages by PageType enum, the underlying math should work for
/// any count
fn test_scalable_multi_page_algorithm() {
    ...
    while i < test_page_counts.len() {
        ...
        assert!(
            expected_total_duration > display_duration,
            "Total duration must exceed display duration",
        );
        println!(
            "  Timing: {}s display + {}s transition = {}s total duration",
            display_duration,
            transition_duration,
    ...
    println!(
        "  - {} pages would require {} seconds ({} minutes)",
    ...
    println!(
        "PASS: No integer overflow in reasonable ranges (up to {} pages)", max_reasonable_pages,
    );
