...
use death_mountain_renderer::utils::renderer_utils::{
    generate_svg, weapon, chest, head, waist, 
    foot, hand, neck, ring, u256_to_string
};
...
use death_mountain_renderer::mocks::mock_adventurer::{
    get_simple_adventurer, get_adventurer_with_max_stats, get_adventurer_with_long_name,
    create_custom_adventurer
};
...
fn test_generate_svg_long_name_font_adjustment() {
    ...
    assert!(contains_pattern(@svg, @"VeryLongAdventurerNameThatEx..."), "Should contain the truncated long name prefix");
}
...
fn test_generate_svg_wounded_health_orange() {
    ...
    assert!(contains_pattern(@svg, @"#FFD700"), "Wounded adventurer should have yellow/gold health bar");
}
...
fn test_generate_svg_equipment_icons() {
    ...
    assert!(contains_pattern(@svg, @"<g transform=\"translate("), "Should contain equipment icon transformations");
