...
fn test_contract_multiple_deployments() {
    ...
    assert_eq!(
        contract1.get_death_mountain_address(), address1, "Contract 1 should have address 1",
    );
    assert_eq!(
        contract2.get_death_mountain_address(), address2, "Contract 2 should have address 2",
    );
...
fn test_contract_with_mock_death_mountain() {
    ...
    assert_eq!(
        renderer_contract.get_death_mountain_address(),
        mock_address,
        "Renderer interface should work",
    );
...
fn test_contract_no_unauthorized_state_changes() {
    ...
    let _description = IMinigameDetailsDispatcher { contract_address }
        .token_description(TEST_TOKEN_ID);
...
fn fuzz_contract_multiple_calls(token_id1: u64, token_id2: u64, token_id3: u64) {
    let (_, contract_address) = deploy_renderer_contract();
...
fn test_contract_interface_inheritance() {
    ...
    assert_eq!(
        minigame_contract.contract_address, contract_address, "IMinigameDetails should match",
    );
