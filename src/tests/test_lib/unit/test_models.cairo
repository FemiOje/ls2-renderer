...
use death_mountain_renderer::mocks::mock_adventurer::{
    get_adventurer_with_max_stats, get_adventurer_with_min_stats, get_simple_adventurer,
};
...
fn test_stats_trait_total_calculation() {
    let stats = Stats {
        strength: 1, dexterity: 2, vitality: 3, intelligence: 4, wisdom: 5, charisma: 6, luck: 7,
    };
...
fn test_equipment_verbose_slot_consistency() {
    let equipment = EquipmentVerbose {
        ...
        ring: ItemVerbose {
            id: 8, name: 'Ring', xp: 30, tier: Tier::T1, item_type: Type::Ring, slot: Slot::Ring,
        },
    };
...
fn test_equipment_empty_slots() {
    let empty_item = ItemVerbose {
        id: 0, name: '', xp: 0, tier: Tier::None, item_type: Type::None, slot: Slot::None,
    };
...
fn fuzz_stats_health_calculation(
    strength: u8, dexterity: u8, vitality: u8, intelligence: u8, wisdom: u8, charisma: u8,
) {
    let stats = Stats { strength, dexterity, vitality, intelligence, wisdom, charisma, luck: 0 };
...
fn fuzz_item_verbose_creation(id: u8, xp: u16) {
    let item = ItemVerbose {
        id,
        name: 'Fuzz Item',
        xp,
