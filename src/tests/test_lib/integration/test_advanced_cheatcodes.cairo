...
use snforge_std::{
    ...
    cheat_block_timestamp, cheat_block_number,
...
fn test_nested_caller_cheats() {
    ...
    assert_eq!(outer_result.len(), restored_result.len(), "Nested caller cheat should restore correctly");
}
...
fn test_block_timestamp_independence() {
    ...
    assert_eq!(result1.len(), result2.len(), "Timestamp should not affect deterministic operations");
}
...
fn test_block_number_independence() {
    ...
    assert_eq!(result1.len(), result2.len(), "Block number should not affect deterministic operations");
}
...
fn test_sequencer_address_independence() {
    ...
    assert_eq!(result_before.len(), result_with_sequencer.len(), "Sequencer address should not affect reads");
    assert_eq!(result_before.len(), result_after.len(), "Should restore correctly after sequencer cheat");
}
...
fn test_account_address_independence() {
    ...
    assert_eq!(result_before.len(), result_with_account.len(), "Account address should not affect reads");
...
fn test_multiple_cheats_combination() {
    ...
    assert_eq!(baseline_result.len(), complex_result.len(), "Multiple cheats should not affect deterministic operations");
}
...
fn test_rapid_cheat_changes() {
    ...
    let callers = array![
        TEST_CALLER_1.try_into().unwrap(),
        TEST_CALLER_2.try_into().unwrap(),
    ];
    ...
    while i < callers.len() {
        ...
        start_cheat_caller_address(contract_address, caller);
        cheat_block_timestamp(contract_address, 1000000 + (i * 100000).try_into().unwrap(), CheatSpan::TargetCalls(1));
