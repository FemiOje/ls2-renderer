...
use death_mountain_renderer::mocks::mock_adventurer::{
    create_custom_adventurer, create_custom_adventurer_with_name, get_adventurer_with_max_stats,
...
fn fuzz_adventurer_health_states(current_health: u16, vitality: u8) {
    ...
    assert!(
        max_health.into() <= 100_u32 + (255_u32 * 15_u32),
        "Max health must not exceed theoretical max",
    );

    if max_health > 0 {
        let health_capped = if current_health > max_health {
            max_health
        } else {
            current_health
        };
...
fn fuzz_adventurer_extreme_values(health: u16, level: u8, all_stats_value: u8) {
    ...
    if all_stats_value == 0 {
        ...
        assert_eq!(
            calculated_health.into(), 100_u32 + (255_u32 * 15_u32), "Max vitality calculation",
        );
    }
}
...
fn fuzz_adventurer_memory_stress(level: u8, health: u16, token_id: u64) {
    ...
    assert_eq!(
        result1.len(), result2.len(), "Consistent inputs should produce consistent output lengths",
    );
}
