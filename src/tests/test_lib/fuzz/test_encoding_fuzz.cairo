...
fn validate_base64(input: @ByteArray, result: @ByteArray) {
    ...
    while i < result.len() {
        let c = result[i];
        let valid = (c >= 65 && c <= 90) || (c >= 97 && c <= 122) || (c >= 48 && c <= 57) || c == 43 || c == 47 || c == 61;
    ...
    let expected_padding = if remainder == 0 { 0 } else { 3 - remainder };
...
fn limit_input(input: ByteArray, max: u32) -> ByteArray {
    ...
    let mut result = "";
    let mut i = 0;
    while i < max { result.append_byte(input[i]); i += 1; }
    result
}
...
fn fuzz_base64_patterns(size: u8, fill: u8) {
    let len: u32 = if size > 50 { 50 } else { size.into() };
    let mut input = "";
    let mut i: u32 = 0;
    while i < len { input.append_byte(fill); i += 1; }
...
fn fuzz_bytes_used_u128(value: u128) {
    ...
    if value == 0 {
        ...
        if value <= 0xFF { assert_eq!(bytes, 1, "1 byte for <=255"); }
        else if value <= 0xFFFF { assert_eq!(bytes, 2, "2 bytes for <=65535"); }
    }
