...
use death_mountain_renderer::utils::string_utils::{
    contains_pattern, starts_with_pattern, ends_with_pattern, is_all_digits
};
...
fn validate_svg_structure(svg: @ByteArray) {
    ...
    assert!(contains_pattern(svg, @"xmlns=\"http://www.w3.org/2000/svg\""), "Should have SVG namespace");
}
...
fn fuzz_svg_stress_test(iterations: u8) {
    let iteration_count: u32 = if iterations > 10 { 10 } else { iterations.into() };
    
    let mut i: u32 = 0;
    while i < iteration_count {
        let adventurer = create_custom_adventurer(
            100 + (i * 10).try_into().unwrap(),
            (i + 1).try_into().unwrap(),
            10
        );
