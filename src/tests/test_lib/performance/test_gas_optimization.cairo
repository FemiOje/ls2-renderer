...
fn test_adventurer_verbose_render_gas_efficiency() {
    ...
    assert!(starts_with_pattern(@simple_result, @"data:application/json;base64,"), "Simple render should produce valid JSON data URI");
    ...
    assert!(starts_with_pattern(@max_result, @"data:application/json;base64,"), "Max stats render should produce valid JSON data URI");
...
fn test_svg_generation_performance_scaling() {
    ...
    assert!(complex_svg.len() >= simple_svg.len(), "Complex SVG should be at least as large as simple");
}
...
fn test_batch_operations_efficiency() {
    ...
    while i < token_ids.len() {
        ...
        assert!(starts_with_pattern(@svg, @"data:image/svg+xml;base64,"), "Each SVG should be valid data URI");
...
fn test_memory_efficiency_multiple_adventurers() {
    ...
    assert!(starts_with_pattern(@result1, @"data:application/json;base64,"), "First render should produce valid JSON data URI");
    
    let adventurer2 = get_adventurer_with_max_stats();
    let result2 = Renderer::render(2_u64, adventurer2);
    assert!(starts_with_pattern(@result2, @"data:application/json;base64,"), "Second render should produce valid JSON data URI");
    
    let adventurer3 = get_simple_adventurer();
    let result3 = Renderer::render(3_u64, adventurer3);
    assert!(starts_with_pattern(@result3, @"data:application/json;base64,"), "Third render should produce valid JSON data URI");
...
fn test_bytes_used_calculation_performance() {
    let test_u128_values = array![0_u128, 255_u128, 65535_u128, 4294967295_u128, 0xffffffffffffffffffffffffffffffff_u128];
...
fn test_adventurer_trait_extraction_performance() {
    ...
    assert!(starts_with_pattern(@simple_image, @"data:image/svg+xml;base64,"), "Should generate valid SVG data URI for simple adventurer");
    assert!(starts_with_pattern(@max_image, @"data:image/svg+xml;base64,"), "Should generate valid SVG data URI for max adventurer");
...
fn test_comparative_operation_analysis() {
    ...
    assert!(starts_with_pattern(@image, @"data:image/svg+xml;base64,"), "Image generation should produce valid SVG data URI");
    
    let render_adventurer = get_simple_adventurer();
    let full_render = Renderer::render(1_u64, render_adventurer);
    assert!(starts_with_pattern(@full_render, @"data:application/json;base64,"), "Full render should produce valid JSON data URI");
    
    assert!(full_render.len() > image.len(), "Full render should be larger than just image");
    assert!(full_render.len() > description.len(), "Full render should be larger than just description");
}
...
fn test_contract_vs_direct_operation_analysis() {
    ...
    assert!(contract_description.len() > 5, "Contract description should produce meaningful content");
    assert!(starts_with_pattern(@contract_svg, @"data:image/svg+xml;base64,"), "Contract SVG should produce valid data URI");
    ...
    assert!(starts_with_pattern(@direct_image, @"data:image/svg+xml;base64,"), "Direct image should produce valid SVG data URI");
}